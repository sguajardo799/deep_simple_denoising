...
def reconstruct_waveform(log_spec: torch.Tensor, original_phase: torch.Tensor, config, device: str) -> torch.Tensor:
    ...
    log_spec: (B, 1, F, T) - Log-magnitude spectrogram (DB scale)
    original_phase: (B, F, T) - Phase of the noisy signal
    """
    # 1. DB to Amplitude
    ...
    if config.transform_type == "spectrogram":
        ...
        # spec_lin shape: (B, n_stft, T)
        # original_phase shape: (B, n_stft, T)
